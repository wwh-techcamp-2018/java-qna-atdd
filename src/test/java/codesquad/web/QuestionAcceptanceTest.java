...
//    }
    ...
    public void create_not_valid() {
       ...
       Question question = new Question("제발1", "되라1");
//        RequestEntity<Question> requestEntity = new
       ResponseEntity<String> response = basicAuthTemplate().postForEntity("/questions", new HttpEntity(question, headers), String.class);
       log.debug("****** {}",response.toString());
       log.debug("Response Body : {}", response.getBody());
       여기까지 application/json */
    ...
    @Test
    public void create_question_fail2() {
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Arrays.asList(MediaType.TEXT_HTML));
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);

        HttpEntity httpEntity = htmlFormDataBuilder.addParameter("title", "")
                .addParameter("contents", "")
                .build();


        ResponseEntity<String> responseEntity = basicAuthTemplate(defaultUser()).postForEntity("/questions/test", httpEntity, String.class);
    }
...
//    @Test
//    public void getResrouces(){
//        Question question = defaultQuestion();
//        HttpEntity httpEntity = htmlFormDataBuilder.delete().build();
//        ResponseEntity<String> responseEntity = getResource(String.format("/questions/%d", question.getId()), ResponseEntity.class, defaultUser());
//        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
//    }
//    @Test
